...
#include "fractional-scale-v1-client-protocol-code.h"
    ...
    #error "PLATFORM_WAYLAND_LAYER requires an OpenGL/OpenGL ES graphics API, software rendering is not supported"
#endif
...
static int InitGraphicsDevice(void);        // Initialize EGL display, context and window surface on the layer surface
static void CloseGraphicsDevice(void);      // Destroy EGL objects and the EGL native window
...
// NOTE: Swap interval is 0 so an unmapped/occluded surface never blocks here,
// frame pacing is left to SetTargetFPS() or EnableEventWaiting()
void SwapScreenBuffer(void)
{
    if ((platform.device == EGL_NO_DISPLAY) || (platform.surface == EGL_NO_SURFACE)) return;

    if (eglSwapBuffers(platform.device, platform.surface) == EGL_FALSE)
    {
        TRACELOG(LOG_WARNING, "DISPLAY: eglSwapBuffers() failed: 0x%04x", eglGetError());
    }

    // Make sure the commit generated by eglSwapBuffers() reaches the compositor right away
...
int InitPlatform(void)
{
    ...
    // Initialize graphics device (EGL context on the layer surface)
    //----------------------------------------------------------------------------
...
// NOTE: Based on rcore_drm.c InitGraphicsDevice(), using the Wayland EGL platform
static int InitGraphicsDevice(void)
{
    ...
    const EGLint renderableType = (rlGetVersion() == RL_OPENGL_ES_30)? EGL_OPENGL_ES3_BIT : EGL_OPENGL_ES2_BIT;
    ...
    #else   // GRAPHICS_API_OPENGL_11, GRAPHICS_API_OPENGL_21: legacy context, any version
    const EGLint contextAttribs[] = {
        EGL_NONE
    };
    #endif
    ...
    const EGLint framebufferAttribs[] = {
        EGL_RENDERABLE_TYPE, renderableType, // Type of context support
        EGL_SURFACE_TYPE, EGL_WINDOW_BIT,   // Window surface (wl_egl_window)
        EGL_RED_SIZE, 8,            // RED color bit depth
        EGL_GREEN_SIZE, 8,          // GREEN color bit depth
        EGL_BLUE_SIZE, 8,           // BLUE color bit depth
        EGL_ALPHA_SIZE, 8,          // ALPHA bit depth, required for transparent bars/panels
        EGL_DEPTH_SIZE, 24,         // Depth buffer size (Required to use Depth testing!)
        //EGL_STENCIL_SIZE, 8,      // Stencil buffer size
        EGL_SAMPLE_BUFFERS, sampleBuffer, // Activate MSAA
        EGL_SAMPLES, samples,       // 4x Antialiasing if activated
        EGL_NONE
    };

    EGLint numConfigs = 0;

    // Get an EGL device connection for the Wayland display
    // NOTE: eglGetPlatformDisplay() is preferred over eglGetDisplay() legacy call
    platform.device = EGL_NO_DISPLAY;
#if defined(EGL_VERSION_1_5)
    platform.device = eglGetPlatformDisplay(EGL_PLATFORM_WAYLAND_KHR, platform.display, NULL);
#else
    ...
    if ((eglClientExtensions != NULL) && (strstr(eglClientExtensions, "EGL_EXT_platform_base") != NULL))
    {
        ...
        if (eglGetPlatformDisplayEXT != NULL) platform.device = eglGetPlatformDisplayEXT(EGL_PLATFORM_WAYLAND_KHR, platform.display, NULL);
    }
#endif

    // In case extension not found or display could not be retrieved, try using legacy version
    if (platform.device == EGL_NO_DISPLAY) platform.device = eglGetDisplay((EGLNativeDisplayType)platform.display);
    ...
    EGLint eglMajor = 0;
    EGLint eglMinor = 0;
    if (eglInitialize(platform.device, &eglMajor, &eglMinor) == EGL_FALSE)
    {
        TRACELOG(LOG_WARNING, "DISPLAY: Failed to initialize EGL device");
        return -1;
    }

    TRACELOG(LOG_INFO, "DISPLAY: EGL %i.%i initialized (%s)", eglMajor, eglMinor, eglQueryString(platform.device, EGL_VENDOR));

    // Get the number of matching configs first
    if (!eglChooseConfig(platform.device, framebufferAttribs, NULL, 0, &numConfigs) || (numConfigs == 0))
    {
    ...
    if (!eglChooseConfig(platform.device, framebufferAttribs, configs, numConfigs, &matchingNumConfigs) || (matchingNumConfigs == 0))
    {
    ...
    // Prefer an exact 8-bit RGBA config: EGL sorts deeper color buffers first, so the first
    // match could be a 10-bit config that the compositor may not handle as expected
    platform.config = configs[0];
    for (EGLint i = 0; i < matchingNumConfigs; i++)
    {
        EGLint red = 0, green = 0, blue = 0, alpha = 0;
        eglGetConfigAttrib(platform.device, configs[i], EGL_RED_SIZE, &red);
        eglGetConfigAttrib(platform.device, configs[i], EGL_GREEN_SIZE, &green);
        eglGetConfigAttrib(platform.device, configs[i], EGL_BLUE_SIZE, &blue);
        eglGetConfigAttrib(platform.device, configs[i], EGL_ALPHA_SIZE, &alpha);
    ...
    {
        TRACELOG(LOG_WARNING, "DISPLAY: Failed to create EGL context: 0x%04x", eglGetError());
        return -1;
    }

    // Create the EGL native window at the physical buffer size computed on configure
    // NOTE: UpdateSurfaceGeometry() already filled CORE.Window.render from logical size and scale
    int bufferWidth = (CORE.Window.render.width > 0)? CORE.Window.render.width : 1;
    int bufferHeight = (CORE.Window.render.height > 0)? CORE.Window.render.height : 1;

    platform.eglWindow = wl_egl_window_create(platform.wlSurface, bufferWidth, bufferHeight);
    if (platform.eglWindow == NULL)
    {
        TRACELOG(LOG_WARNING, "DISPLAY: Failed to create wl_egl_window");
        return -1;
    }
    ...
    platform.surface = EGL_NO_SURFACE;
#if defined(EGL_VERSION_1_5)
    platform.surface = eglCreatePlatformWindowSurface(platform.device, platform.config, platform.eglWindow, NULL);
#else
    if ((eglClientExtensions != NULL) && (strstr(eglClientExtensions, "EGL_EXT_platform_base") != NULL))
    {
        ...
        if (eglCreatePlatformWindowSurfaceEXT != NULL) platform.surface = eglCreatePlatformWindowSurfaceEXT(platform.device, platform.config, platform.eglWindow, NULL);
    }
#endif

    if (platform.surface == EGL_NO_SURFACE)
    {
        platform.surface = eglCreateWindowSurface(platform.device, platform.config, (EGLNativeWindowType)platform.eglWindow, NULL);
    }
    ...
    {
        TRACELOG(LOG_WARNING, "DISPLAY: Failed to make EGL context current: 0x%04x", eglGetError());
        return -1;
    }

    // Never block in eglSwapBuffers(): the compositor may not be presenting the surface
    // (unmapped, on a disabled output...), frame pacing is handled by raylib timing
    eglSwapInterval(platform.device, 0);
...
// Destroy EGL objects and the EGL native window
static void CloseGraphicsDevice(void)
{
    ...
    if (platform.eglWindow != NULL)
    {
        wl_egl_window_destroy(platform.eglWindow);
        platform.eglWindow = NULL;
    }
